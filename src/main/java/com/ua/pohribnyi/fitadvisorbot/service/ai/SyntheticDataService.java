...
public class SyntheticDataService {
    ...
    /**
     * This is the main entry point, called synchronously by the Handler.
     * It creates the Job and triggers the *first* async worker.
     */
    @Transactional
    public void triggerHistoryGeneration(User user, UserProfile profile) {
        log.info("Creating PENDING job for user {}", user.getId());
        
        GenerationJob job = GenerationJob.createPendingJob(user);
